...
- [Swift初窥](#swift初窥)
    ...
    - [模式](#模式)
        ...
        - [表达式模式](#表达式模式)
        - [类型转换模式](#类型转换模式)
    - [Swift与Objective-C的混编](#swift与objective-c的混编)
...
* 构造器的代理:构造器通过调用其它构造器来完成实例的部分构造过程.
    ...
    * 类类型:`类类型`除了可以代理自己的其他构造方法外,还可以继承自其它类.
        * 指定构造器:`指定构造器`是类中最主要的构造器,其将初始化类中提供的所有属性,并通过调用父类的构造器来实现父类的初始化.  
        **每一个类都必须拥有至少一个`指定构造器`.**
        * 便利构造器:类中比较次要的,辅助型构造器.使用`convenience`关键字修饰.
...
如果希望`默认构造器`,`逐一成员构造器`和用户自定义构造器共存,都能用来创建实例,可以将用户自定义构造器写到[扩展]()(`extension`)中.

TODO:FIXME:[两段式构造过程](http://wiki.jikexueyuan.com/project/swift/chapter2/14_Initialization.html#class_inheritance_and_initialization)
